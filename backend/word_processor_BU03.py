...
def create_run_element_with_text(text_content, template_run_r=None, is_del_text=False, highlight_color: Optional[str] = None):
    ...
    # Try to copy existing rPr from template
    if template_run_r is not None:
        rPr_template = template_run_r.find(qn('w:rPr'))
        if rPr_template is not None:
            rPr_element = copy.deepcopy(rPr_template)
    
    # Ensure rPr_element exists if we need to add highlighting or if it was copied
    if rPr_element is None and highlight_color:
        rPr_element = OxmlElement('w:rPr')
    elif rPr_element is None: # No template rPr and no highlight, rPr might not be needed
        pass 
    ...
    if rPr_element is not None and rPr_element.childNodes: # Only append rPr if it has content or was copied meaningfully
...
def replace_text_in_paragraph_with_tracked_change(
    ...
    for p_child_idx_loop, p_child_element in enumerate(paragraph._p):
        text_contribution = ""
        element_type = "other"
    ...
    try:
        ...
        ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "visible_text_snippet": visible_paragraph_text_original_case[:100],"contextual_old_text": contextual_old_text, "specific_old_text": specific_old_text,"llm_reason": reason_for_change, "issue": f"Regex error: {e}"})
    ...
    if len(occurrences) > 1:
        ...
        log_debug(log_message)
        ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "visible_text_snippet": visible_paragraph_text_original_case[:100],"contextual_old_text": contextual_old_text, "specific_old_text": specific_old_text,"specific_new_text": specific_new_text, "llm_reason": reason_for_change,"issue": f"Ambiguous context: Found {len(occurrences)} occurrences."})
    ...
    for item in elements_contributing_to_visible_text:
        ...
        if max(item['doc_start_offset'], global_specific_start_offset) < min(item_doc_end_offset, global_specific_end_offset):
            ...
            if first_involved_r_element_for_style is None:
                first_involved_r_element_for_style = _get_element_style_template_run(item, None) # Pass None first
    ...
    if not involved_element_infos:
        ...
        log_debug(log_message)
        ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "issue": "XML mapping failed for specific text.", "contextual_old_text": contextual_old_text, "specific_old_text": actual_specific_old_text_to_delete, "llm_reason": reason_for_change,"visible_text_snippet": visible_paragraph_text_original_case[:100]})
    ...
    log_debug(f"P{paragraph_idx+1}: Modifying {len(involved_element_infos)} raw XML elements.")

    # Determine if highlighting is needed for the insertion
    ...
    for p_idx_to_remove_loop in p_child_indices_to_remove:
        try:
            paragraph._p.remove(paragraph._p[p_idx_to_remove_loop])
            log_debug(f"P{paragraph_idx+1}: Removed original XML element at p_child_idx {p_idx_to_remove_loop}.")
        except IndexError:
            log_message = f"P{paragraph_idx+1}: XML element removal error at index {p_idx_to_remove_loop}. Change aborted. LLM Reason: {reason_for_change}"
            log_debug(log_message)
            ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "issue": f"XML element removal error at index {p_idx_to_remove_loop}.","contextual_old_text": contextual_old_text, "specific_old_text": actual_specific_old_text_to_delete,"llm_reason": reason_for_change})
    ...
    if add_comments_flag and reason_for_change:
        ...
        try:
            ...
            ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "issue": f"Comment addition failed (AttributeError): {e_attr}. Paragraph object may be unstable.","contextual_old_text": contextual_old_text, "specific_old_text": actual_specific_old_text_to_delete,"specific_new_text": specific_new_text, "llm_reason": reason_for_change, "type": "CriticalWarning"})
        except Exception as e_gen:
            log_debug(f"P{paragraph_idx+1}: WARNING - Could not add comment. Error: {e_gen}")
            ambiguous_or_failed_changes_log.append({"paragraph_index": paragraph_idx + 1, "issue": f"Comment addition failed: {e_gen}","contextual_old_text": contextual_old_text, "specific_old_text": actual_specific_old_text_to_delete,"specific_new_text": specific_new_text, "llm_reason": reason_for_change, "type": "Warning"})
    return "SUCCESS"
...
def process_document_with_edits(
    ...
    for para_idx, paragraph in enumerate(doc.paragraphs):
        # Check if we need to attempt further edits based on the original number of edits.
        # This simplistic check assumes one edit processed corresponds to one item from edits_to_make.
        # If an edit is skipped for a paragraph, other edits might still be applicable to it.
        # The 'break' inside replace_text_in_paragraph_with_tracked_change limits one SUCCESS per paragraph scan.
        
        # Create a list of edits yet to be successfully applied or definitively failed for this paragraph
        # This is complex if an edit is applicable to multiple paragraphs.
        # For now, iterate all edits for each paragraph, but rely on the break for stability.
        
        original_paragraph_text_for_this_pass = paragraph.text # For debugging or more complex edit management
        log_debug(f"\n--- Processing Paragraph {para_idx+1} (Text: '{original_paragraph_text_for_this_pass[:50]}...') ---")

        for edit_item_idx, edit_item in enumerate(list(edits_to_make)): # Iterate a copy if edits_to_make can be modified
            # A more robust system might track which edits have been successfully applied globally
            # and not re-attempt them, or re-evaluate applicability if paragraph text changes.
            # Current 'break' on SUCCESS handles stability for one paragraph pass.
            ...
            if not all(key in edit_item for key in required_keys):
                ...
                log_debug(log_message)
                ambiguous_or_failed_changes_log.append({"paragraph_index": para_idx + 1, "edit_item_index": edit_item_idx +1, "issue": "Invalid edit item structure.", "edit_item_snippet": str(edit_item)[:100]})
                continue

            specific_new_text = edit_item.get("specific_new_text", "")
            
            # It's critical to re-evaluate the paragraph's text and structure if multiple changes
            # are to be applied to THE SAME paragraph. The current code with `break` on SUCCESS
            # only applies the first successful edit to a given paragraph during its scan.
            # If `replace_text_in_paragraph_with_tracked_change` is called multiple times
            # on the same paragraph object after modifications, `visible_paragraph_text_original_case`
            # inside it needs to be rebuilt based on the *current* state of `paragraph._p`.
            # The current implementation rebuilds it fresh each call, which is good.
            ...
            except Exception as e_replace: # Catch unexpected errors from the replacement function
                status = "UNHANDLED_EXCEPTION_IN_REPLACE"
                log_message = f"P{para_idx+1}, EditItem{edit_item_idx+1}: Unhandled exception during replacement for '{edit_item['specific_old_text']}'. Error: {e_replace}"
                log_debug(log_message)
                # import traceback # For more detailed stack trace
                # log_debug(traceback.format_exc())
                ambiguous_or_failed_changes_log.append({
                    "paragraph_index": para_idx + 1, 
                    "edit_item_index": edit_item_idx +1,
                    "issue": f"Unhandled exception in replacement: {e_replace}", 
            ...
            if status == "SUCCESS":
                ...
                # After a successful change, the paragraph's XML structure is modified.
                # For stability, break from processing further edits *on this specific paragraph object* in this pass.
                # The paragraph will be re-read from doc.paragraphs if the outer loop continues or on a new function call.
                break 
        ...
        try:
            with open(error_log_file_path, "w", encoding="utf-8") as f:
                ...
                for log_entry in ambiguous_or_failed_changes_log:
                    ...
                    log_type = log_entry.get('type', 'Error') # Default to Error if not specified
...
if __name__ == '__main__':
    import argparse
    # ( argparse and __main__ block largely unchanged from your provided script )
    # ( ... ensure it calls process_document_with_edits correctly ... )
    DEFAULT_EDITS_FILE_PATH = "edits_to_apply.json"
    DEFAULT_INPUT_DOCX_PATH = "sample_input.docx" 
    DEFAULT_OUTPUT_DOCX_PATH = "sample_output_highlighted.docx"
    ...
    if args.editsjson:
        ...
        if not DEBUG_MODE : DEBUG_MODE = True # Enable debug if run standalone with no edits for info
        log_debug("No edits provided via --editsfile or --editsjson. Will try to run with dummy edits if input/output are defaults.")
        # Fallback to dummy edits for basic testing if no edits provided
        if args.input == DEFAULT_INPUT_DOCX_PATH and args.output == DEFAULT_OUTPUT_DOCX_PATH :
            ...
            edits_data = [
                { # Example that should highlight due to no trailing space before comma
                    "contextual_old_text": "change this sentence from saying the cost would be $101 , to a new number",
                    "specific_old_text": "$101", 
                    "specific_new_text": "$555",
                    "reason_for_change": "Test currency change, expect highlight."
                },
                { # Example that should NOT highlight (surrounded by spaces)
                    "contextual_old_text": "The last line was last edited by MrArbor, but that name can change",
                    "specific_old_text": "name",
                    "specific_new_text": "designation",
                    "reason_for_change": "Test word change, no highlight expected."
                },
                 { # Example that should highlight (end of sentence)
                    "contextual_old_text": "long long long long repetitive line with 9 longs.",
                    "specific_old_text": "longs.",
                    "specific_new_text": "iterations!",
                    "reason_for_change": "Test end of sentence, expect highlight."
                }
            ]
            # Create a dummy input if it doesn't exist for this test
            if not os.path.exists(DEFAULT_INPUT_DOCX_PATH):
                try:
                    doc = Document()
                    doc.add_paragraph("A simple file. It should change this sentence from saying the cost would be $101 , to a new number.")
                    doc.add_paragraph("Another line. Lets count 1, 2, 3.")
                    doc.add_paragraph("Here is a long long long long long long long long long long repetitive line with 9 longs.")
                    doc.add_paragraph("The last line was last edited by MrArbor, but that name can change.")
